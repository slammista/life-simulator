// WorkNpcDetailModal — BitLife-style bottom sheet for a colleague/superior.
// Mirrors PersonDetailModal but works with WorkNPC instead of Relationship.

import { ensureNpcAttributesById, NPC_ATTR_META } from '../../services/NpcAttributes'
import type { WorkNPC, WorkAction, NPCPersonalityTrait } from '../../store/types'
...
const LEVEL_LABELS: Record<WorkNPC['level'], string> = {
  colleague: '👥 Collega',
  superior:  '👔 Superiore',
  ceo:       '🏢 CEO',
}

const WORK_ACTIONS: Array<{ action: WorkAction; label: string; emoji: string; desc: string; bg: string }> = [
  { action: 'talk',       label: 'Parla',       emoji: '💬', desc: 'Fai due chiacchiere con lui/lei',         bg: '#60a5fa' },
  { action: 'socialize',  label: 'Esci',        emoji: '☕', desc: 'Uscite insieme dopo il lavoro',           bg: '#3b82f6' },
  { action: 'help',       label: 'Aiuta',       emoji: '🤝', desc: 'Dagli/le una mano con il lavoro',        bg: '#10b981' },
  { action: 'compliment', label: 'Loda',        emoji: '😊', desc: 'Fagli/le un complimento professionale',  bg: '#f59e0b' },
  { action: 'gossip',     label: 'Gossip',      emoji: '🗣️', desc: 'Spettegola insieme su altri colleghi', bg: '#f472b6' },
  { action: 'fight',      label: 'Litiga',      emoji: '😠', desc: 'Litigate in ufficio',                    bg: '#ef4444' },
]

interface Props {
  npc: WorkNPC
  onClose: () => void
  onInteract: (npcId: string, action: WorkAction) => void
}

export function WorkNpcDetailModal({ npc, onClose, onInteract }: Props) {
    ...
    <div style={overlay} onClick={onClose}>
      <div style={sheet} onClick={e => e.stopPropagation()}>
        ...
        <div style={topBar}>
          ...
          <div style={{ flex: 1, minWidth: 0, textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)', margin: 0, marginTop: 1 }}>
              {LEVEL_LABELS[npc.level]} · {npc.age} anni
            </p>
          </div>
        ...
        <div style={{ marginLeft: -16, marginRight: -16 }}>
          {WORK_ACTIONS.map(({ action, label, emoji, desc, bg }) => (
            <div
              key={action}
