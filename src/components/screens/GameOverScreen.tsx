...
export function GameOverScreen() {
  ...
  const { deathType, time, stats, finance, completedGoals, goals, newGame, identity, continueAsChild, legacy } = store
            ...
            return (
              <div key={key} style={{ marginBottom: 6 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, fontSize: 12, marginBottom: 2 }}>
                  <span style={{ color: 'var(--color-text-secondary)', minWidth: 0 }}>{labels[key]}</span>
                  <span style={{ fontWeight: 600, flexShrink: 0 }}>{val}/{maxes[key]}</span>
                </div>
